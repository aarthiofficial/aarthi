...
public class PlayerService implements PlayerRepository {
    ...
    public Optional<Player> findPlayerById(int playerId) {
        return Optional.ofNullable(team.get(playerId));
    }
    ...
    public Player updatePlayer(int playerId, Player updatedPlayer) {
        ...
        team.replace(playerId, updatedPlayer);
        return updatedPlayer;
    }

    // src/main/java/com/example/player/PlayerService.java
    ...
    public void deletePlayer(int playerId) {
        if (!team.containsKey(playerId)) {
            ...
            team.remove(playerId); // Remove the player from the team
        }
    }
}